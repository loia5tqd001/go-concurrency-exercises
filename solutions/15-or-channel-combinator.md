...
- **Goroutine count.** Approach 1 spawns one goroutine per recursion level. Because each level peels off exactly 2 channels (`channels[0]`, `channels[1]`) and folds the remaining N-2 back together with 1 new `orDone` into the recursive call, the next level always gets N-1 channels, not N/2 — so recursion depth, and therefore total goroutine count, is exactly **N-1** for N channels, not a halving at each step. Approach 2 spawns exactly **one** goroutine total, regardless of N, blocked in a single `reflect.Select` over a flat, N-case list. There's no recursive tail to worry about leaking, because there's no recursion — the orphaned-goroutine trap Approach 1 has to explicitly work around doesn't exist here in the first place.
- **Shape of the wait.** Approach 1 is a linear O(N)-deep *chain* of two-way selects, not a balanced O(log N) tree — despite "divide-and-conquer" framing, it peels off a constant 2 channels per level rather than splitting the list in half. Approach 2 is a single O(N) *flat* fan-in built once via reflection.
